...
export default class ProductForm {
  ...
  getCategoryOptionsTemplate(categories, catTitle = []) {
    ...
    return categories.map(value => {
      catTitle.push(value.title); //тут собираем тайтл для options
  ...
  uploadFile = async (event) => {
    ...
    try {
      ...
      this.sortableListComponent.element.insertAdjacentHTML('beforeend', this.getImgItemText(newImgItem));

    } catch (error) {
      console.log(error); //ошибка помещения файла на сервис
    }
  ...
  fillFormData(data) {
    ...
    this.sortableListComponent.element.innerHTML = this.data.images.map(value => {
