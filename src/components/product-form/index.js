...
export default class ProductForm {
  ...
  constructor(productId) {
    ...
    this.urlCategory = new URL('api/rest/categories', process.env.BACKEND_URL);
    this.urlCategory.searchParams.set('_sort', 'weight');
    this.urlCategory.searchParams.set('_refs', 'subcategory');
    this.urlProduct = new URL('api/rest/products', process.env.BACKEND_URL);
    this.urlProductGet = new URL('api/rest/products', process.env.BACKEND_URL);
  ...
  async loadData() {
    ...
    const [resultCategory, resultProduct] = await Promise.all([
      resultCategoryPromise,
      resultProductPromise
    ]);
    ...
    if (this.categories) {
      for (const category of this.categories) {
        for (const subCategory of category.subcategories) {
          const newOption = new Option(`${category.title} > ${subCategory.title}`, subCategory.id);
    ...
    wrapper.innerHTML = `
      <li class="products-edit__imagelist-item sortable-list__item" style="">
        ...
        <span>
          ...
          <img class="sortable-table__cell-img" alt="${escapeHtml(source)}" src="${escapeHtml(
      url
    )}">
  ...
  loadImages() {
    const items = this.resultProduct.images.map(({ url, source }) =>
