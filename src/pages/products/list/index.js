...
export default class Page {
  ...
  getFilterData() {
    //api/rest/products?_embed=subcategory.category&price_gte=0&price_lte=1043&_sort=title&_order=asc&_start=0&_end=30
    const baseURL = new URL(
      `${process.env.BACKEND_URL}api/rest/products?_embed=subcategory.category&_sort=title&_order=asc`
    );
  ...
  initEventListeners() {
    ...
    this.subElements.filterForm.addEventListener('submit', event => {
      ...
      this.currentFilter.status =
        formData.get('filterStatus') === '' ? null : Number(formData.get('filterStatus'));
